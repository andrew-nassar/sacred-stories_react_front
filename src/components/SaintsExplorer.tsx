...
import { Search, Filter, Sparkles, BookOpen, MapPin, Award, Calendar, Quote, BookOpenCheck, RefreshCw } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { SAINTS_DATA, Saint } from "../data";

interface SaintsExplorerProps {
  onSaintClick: (saint: Saint) => void;
  initialSearchQuery?: string;
  onClearInitialSearch?: () => void;
}

export default function SaintsExplorer({ onSaintClick, initialSearchQuery = "", onClearInitialSearch }: SaintsExplorerProps) {
  ...
  // Dynamic saint synthesis via server proxy
  const handleSynthesizeSaint = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || data.error || "Failed to search the heavenly archives.");
      }
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex gap-3 justify-center md:justify-start">
                  <button
                    onClick={() => onSaintClick(synthesizedSaint)}
                  ...
                  <button
                    onClick={() => {
                      // Reset synthesis to allow another
                      setSynthesizedSaint(null);
                    }}
