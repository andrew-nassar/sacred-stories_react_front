...
import { TIMELINE_DATA, SAINTS_DATA, TimelineEvent, Saint } from "../data";
import { Calendar, ChevronRight, Compass, Info } from "lucide-react";
...
export default function TimelineSection({ onSaintSelect }: TimelineSectionProps) {
  ...
  const handleTimelineItemClick = (event: TimelineEvent, index: number) => {
    ...
    if (event.saintId) {
      const match = SAINTS_DATA.find((s) => s.id === event.saintId);
      if (match) {
        // Option to go directly to saint details
      }
    }
  };
        ...
        <div className="space-y-12">
          {TIMELINE_DATA.map((event, index) => {
            ...
            return (
              ...
              >
                {/* Year Label (Absolute on Desktop, absolute-left on Mobile) */}
                    ...
                    <div className="mt-4 pt-3 border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          onSaintSelect(associatedSaint);
                        }}
                        className="self-start text-[11px] font-mono text-gold-accent hover:text-white flex items-center gap-1 transition-all"
                      >
